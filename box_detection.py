...
def generate_grid_points(point1_x, point1_y, point2_x, point2_y, spacing, frame_number=0, width=1920, height=1080, region_idx=0):
    ...
    print(f"Generated {len(queries)} grid points for region {region_idx} with spacing {spacing:.2f}, "
          f"Box coords: top_left=({min_x}, {min_y}), bottom_right=({max_x}, {max_y})")
...
def custom_visualize(video, tracks, visibility, regions, save_path, fps=30):
    ...
    print(f"Visualizing {len(regions)} regions with square boxes")
    ...
    for t in range(num_frames):
        ...
        for region_idx, region in enumerate(regions):
            ...
            if t < len(frame_positions) and frame_positions[t]['mean_x'] is not None:
                ...
                # 计算初始框的宽度和高度
                init_top_left = region['box_coords']['top_left']
                init_bottom_right = region['box_coords']['bottom_right']
                box_width = init_bottom_right[0] - init_top_left[0]
                box_height = init_bottom_right[1] - init_top_left[1]
                
                # 更新框的坐标，保持框大小不变
                new_top_left = (mean_x - box_width / 2, mean_y - box_height / 2)
                new_bottom_right = (mean_x + box_width / 2, mean_y + box_height / 2)
                
                # 确保框在视频边界内
                new_top_left = (
                    max(0, min(new_top_left[0], width - 1)),
                    max(0, min(new_top_left[1], height - 1))
                )
                new_bottom_right = (
                    max(0, min(new_bottom_right[0], width - 1)),
                    max(0, min(new_bottom_right[1], height - 1))
                )
                ...
                cv2.rectangle(
                    frame,
                    (int(new_top_left[0]), int(new_top_left[1])),
                    (int(new_bottom_right[0]), int(new_bottom_right[1])),
                    color,
                    thickness=2
                )
                print(f"Frame {t}, Region {region_idx}: Box at top_left=({new_top_left[0]:.2f}, {new_top_left[1]:.2f}), "
                      f"bottom_right=({new_bottom_right[0]:.2f}, {new_bottom_right[1]:.2f})")
            else:
                # 如果没有有效点，绘制初始框
                top_left = region['box_coords']['top_left']
                bottom_right = region['box_coords']['bottom_right']
                color = REGION_COLORS[region_idx % len(REGION_COLORS)]
                cv2.rectangle(
                    frame,
                ...
                print(f"Frame {t}, Region {region_idx}: No valid points, using initial box")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--points", type=float, nargs='+', default=[930, 880, 1080, 1030], 
    ...
    print(f"Regions: {len(args.points)//4} regions, Points: {args.points}, Spacing={args.spacing}")
    ...
    try:
        for video_chunk in read_video_with_ffmpeg_chunked(args.video_path, chunk_size=args.chunk_size):
            ...
            # 可视化
            print("Visualizing chunk with square region boxes")
