...
def custom_visualize(video, tracks, visibility, regions, save_path, threshold=0.5, fps=30, crop_region=None):
    ...
    参数:
        ...
        threshold: 移动检测阈值
        fps: 输出视频帧率
        crop_region: 裁剪区域 (x1, y1, x2, y2)，或 None
    """
    ...
    for t in range(num_frames):
        ...
        if crop_region is not None:
            ...
            cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 255, 255), 2)  # 蓝色框，粗细 2
            if t == 0:
                print(f"帧 {t}: 绘制裁剪区域框 [{x1}, {y1}, {x2}, {y2}]")
...
if __name__ == "__main__":
    print("\n" + '-' * 10 + " 程序开始 " + '-' * 10)
    parser = argparse.ArgumentParser(description="视频点跟踪，支持裁剪区域和可视化")
    ...
    parser.add_argument("--crop", type=float, nargs=4, metavar=('x1', 'y1', 'x2', 'y2'),
                        help="裁剪区域坐标 x1,y1,x2,y2 (左上角和右下角)")
    ...
    if args.crop:
        print(f"裁剪区域: x1={args.crop[0]}, y1={args.crop[1]}, x2={args.crop[2]}, y2={args.crop[3]}")
    ...
    if args.crop:
        ...
        if not (0 <= x1 < x2 <= width and 0 <= y1 < y2 <= height):
            raise ValueError(f"裁剪区域 [{x1}, {y1}, {x2}, {y2}] 无效，视频尺寸为 {width}x{height}")
    ...
    try:
        for video_chunk in read_video_with_ffmpeg_chunked(args.video_path, chunk_size=args.chunk_size):
            ...
            # 自定义可视化，传递裁剪区域
            print("使用自定义颜色、动态线条、距离文本、裁剪区域框和点坐标可视化块")
